...
def cast_stream_types(streams_df):
    ...
    Rows that fail either cast (and were not already null) are dropped.
...
def build_kpis(streams_df, songs_df, logger=None):
    ...
    log("Dropped %d stream row(s) with un-castable user_id/listen_time." % dropped)
...
def main():
    ...
    args = getResolvedOptions(
        # sys.argv is read internally by getResolvedOptions
        __import__("sys").argv,
        ["JOB_NAME", "raw_bucket", "reference_bucket", "processed_bucket"],
    )
    ...
    final = build_kpis(streams_df, songs_df, logger=logger)
